...
function Boards({ currentUser }) {
  ...
  const boardAtual = boards.find((b) => b.id === boardIdAtivo) || {
    name: "LOADING...",
  };
    ...
    async function loadPosts() {
      try {
        ...
        const response = await fetch(
          `${API_BASE}/boards/${boardIdAtivo}/posts/`,
        );
        const data = await response.json();
        setPosts(data);
      } catch (error) {
  ...
  useEffect(() => {
    async function loadBoardMembers() {
      try {
        ...
        const response = await fetch(
          `${API_BASE}/boards/${boardIdAtivo}/members/`,
        );
        const data = await response.json();
  ...
  const handleCreateBoard = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "The Django server rejected the creation. Status:",
          response.status,
        );
      }
    } catch (error) {
  ...
  return (
    <div className="home-layout-grid" style={{ position: "relative" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {boards.map((b) => (
            <Link
              ...
              key={b.id}
              className={
                b.id === boardIdAtivo ? "board-box active" : "board-box"
              }
              style={{
                textDecoration: "none",
                width: "100%",
      ...
      <div style={{ flex: "2", minWidth: "0" }}>
        ...
        {currentUser &&
          boardAtual &&
          boardAtual.creator === currentUser.id &&
          !boardAtual.public && (
            <div
              className="polls-box-placeholder"
              style={{ marginBottom: "10px" }}
            >
                ...
                >
                  ...
                  >
                    ...
                    {users
                      .filter(
                        (user) =>
                          !boardMembers.find((member) => member.id === user.id),
                      )
                      .map((user) => (
                  ...
                  {memberSuccess && (
                    <p
                      style={{ fontSize: "11px", color: "#00ff66", margin: 0 }}
                    >
                      {memberSuccess}
                    </p>
                  )}
              ...
              {!showMemberAdder && (
                ...
                >
                  <p style={{ fontSize: "11px", color: "#888896", margin: 0 }}>
                    Members ({boardMembers.length})
                  </p>
                  {boardMembersLoading ? (
                    <p
                      style={{ fontSize: "11px", color: "#888896", margin: 0 }}
                    >
                      Loading members...
                    </p>
                    ...
                    <p
                      style={{ fontSize: "11px", color: "#555566", margin: 0 }}
                    >
                      No members yet
                    </p>
                  )}
        ...
        <div className="posts-list-container">
          {postsLoading ? (
            <p style={{ color: "#888896", fontSize: "13px", padding: "20px" }}>
              A carregar posts...
            </p>
          ) : posts.length > 0 ? (
            posts.map((post) => (
              ...
              >
                <div className="post-main-info">
                  <span
                    ...
                    style={{
                      color: boardById[post.board]?.color || BOARD_COLORS[0],
                      borderColor:
                        boardById[post.board]?.color || BOARD_COLORS[0],
                    }}
                  >
                  ...
                  <p className="post-meta">
                    ...
                    {renderUsername(
                      post.creator,
                      post.creator_username || `User ${post.creator}`,
                      handleOpenUserProfile,
                      "post-author",
                    )}
                  </p>
            ...
            >
              No posts found in this category yet. Click "Back to Home" to view
            ...
            >
              <div>
                <span
                  ...
                  style={{
                    color:
                      boardById[selectedPost.board]?.color || BOARD_COLORS[0],
                    borderColor:
                      boardById[selectedPost.board]?.color || BOARD_COLORS[0],
                    fontSize: "10px",
                  }}
                >
                  {boardById[selectedPost.board]?.name ||
                    `Board ${selectedPost.board}`}
                </span>
                <h2
                  style={{
                  ...
                  comments.map((comment) => (
                    <div
                      ...
                      style={{
                        display: "flex",
                        flexDirection: "column",
                        marginLeft: comment.parent_comment ? "18px" : 0,
                      }}
                    >
                      <div style={{ paddingLeft: "4px" }}>
                        ...
                        >
                          {renderUsername(
                            comment.user,
                            comment.user_username || `User ${comment.user}`,
                            handleOpenUserProfile,
                          )}
                          {" • "}
                          {comment.creation_date}
                          {comment.parent_comment
                            ? ` • reply to ${comment.parent_comment}`
                            : ""}
                        </p>
                        <p
                          style={{
            ...
            >
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                <input
                  type="text"
                  placeholder={
                    currentUser ? "Write a comment..." : "Login para comentar"
                  }
                  className="home-search-input"
                  style={{
                    background: "#110c1c",
                  ...
                  disabled={!currentUser}
                />
                <button className="refresh-button" disabled={!currentUser}>
                  Post
                </button>
              </div>
            </form>
